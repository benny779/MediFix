...
import { List, Typography, Grid, Box, ListItem, IconButton, Chip } from '@mui/material';
...
const PractitionerServiceCalls = () => {
  ...
  return isLoading ? (
    ...
    <Grid item xs={12} md={6}>
      ...
      <List sx={{ padding: 0 }}>
        {response.serviceCalls.map((item) => {
          const { location } = item;
          ...
          return (
            <ListItem
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1 }}>
                <Chip
                  icon={type === 'Repair' ? <BuildIcon /> : <AddCircleOutlineIcon />}
                  label={type}
                  size='small'
